...
#    localStd_map = np.sqrt(localVar_map)
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            cg = 0.8*meansGlobal/ localStd_map[i,j];
            if cg >maxCg:
                cg=maxCg
            # elif cg<1:
            #     cg=1
...
def main():
    ...
    #         if dstimg[i][j][0] - img[i][j][0] != 0:
    #             print("{}, {}: {}".format(i, j, dstimg[i][j][0] - img[i][j][0]))
    cv2.imwrite('output.png',dstimg)
    cv2.waitKey(0)
    
    return 0
 
    
input_fn='./fh_outdoor_gradimg/gradimg3.png'
